...
import { EditorialLines, editorialLinesToText, textToEditorialLines } from "@/components/content-text";
...
function EditorialContentArea({ label, value, onChange, required, max = 250 }: { label: string; value: string; onChange: (value: string) => void; required?: boolean; max?: number }) {
  const lines = textToEditorialLines(value);
  const error = required && !value.trim() ? `${label} is required.` : value.length > max ? `Keep this below ${max} characters.` : undefined;
  return (
    <Field label={label} required={required} hint="Use *text* for cursive emphasis. Enter for line breaks." error={error} count={`${value.length}/${max}`}>
      <TextArea value={value} onChange={(event) => onChange(event.target.value)} maxLength={max + 40} />
      {value.trim() && (
        <div className="admin-headline-preview" aria-live="polite">
          <span className="admin-headline-preview__label">Live preview</span>
          <div className="admin-headline-preview__content">
            <EditorialLines lines={lines} />
          </div>
        </div>
      )}
    </Field>
...
export function HomeContentEditor({ content, media }: { content: HomePageContent; media: MediaAsset[] }) {
  const [copy, setCopy] = useState({
    ...
    heroHeading: editorialLinesToText(content.hero.heading),
    primaryLabel: content.hero.primaryCTA.label,
    secondaryLabel: content.hero.secondaryCTA.label,
    capabilityHeading: editorialLinesToText(content.capabilityIntro.heading),
    ...
    experienceHeading: editorialLinesToText(content.experience.heading),
    experienceBody: content.experience.body,
    finalHeading: editorialLinesToText(content.finalCTA.heading),
  ...
  const update = (key: keyof typeof copy, value: string) => setCopy((current) => ({ ...current, [key]: value }));
  const document = (): HomePageContent => ({
    ...content,
    hero: { ...content.hero, meta: (() => { const [first, second] = copy.heroMeta.split("·").map((item) => item.trim()); return [first || content.hero.meta[0], second || content.hero.meta[1]] as [string, string]; })(), kicker: copy.heroKicker, heading: textToEditorialLines(copy.heroHeading), primaryCTA: { ...content.hero.primaryCTA, label: copy.primaryLabel }, secondaryCTA: { ...content.hero.secondaryCTA, label: copy.secondaryLabel }, media: media.find((asset) => asset.id === heroMediaId) ?? content.hero.media },
    capabilityIntro: { ...content.capabilityIntro, heading: textToEditorialLines(copy.capabilityHeading), body: copy.capabilityBody },
    ...
    experience: { ...content.experience, heading: textToEditorialLines(copy.experienceHeading), body: copy.experienceBody },
    finalCTA: { ...content.finalCTA, heading: textToEditorialLines(copy.finalHeading), body: copy.finalBody },
  });
    ...
    <div className="admin-editor-layout">
      <div>
        <FormSection eyebrow="01 / HERO" title="The first impression" description="The headline is intentionally short so it retains its editorial scale on every screen.">
          ...
          <EditorialContentArea label="Headline" value={copy.heroHeading} onChange={(value) => trackedUpdate("heroHeading", value)} required max={80} />
        ...
        <FormSection eyebrow="02 / CAPABILITY INTRODUCTION" title="Supply context" description="Introduces the scale and requirement-led nature of the business without presenting a fixed catalogue.">
          <EditorialContentArea label="Heading" value={copy.capabilityHeading} onChange={(value) => trackedUpdate("capabilityHeading", value)} required max={120} /><ContentArea label="Body copy" value={copy.capabilityBody} onChange={(value) => trackedUpdate("capabilityBody", value)} required max={300} />
        </FormSection>
        ...
        <FormSection eyebrow="08 / EXPERIENCE" title="New venture, established experience" description="This distinction must remain factual: Star Energies is a new business backed by industry experience.">
          <div className="admin-fields admin-fields--two"><EditorialContentArea label="Heading" value={copy.experienceHeading} onChange={(value) => trackedUpdate("experienceHeading", value)} required max={120} /><ContentArea label="Body copy" value={copy.experienceBody} onChange={(value) => trackedUpdate("experienceBody", value)} required max={300} /></div>
        </FormSection>
        ...
        <FormSection eyebrow="09 / FINAL CALL TO ACTION" title="Enquiry moment" description="Keeps direct contact available as an alternative to the quote form.">
          <div className="admin-fields admin-fields--two"><EditorialContentArea label="Heading" value={copy.finalHeading} onChange={(value) => trackedUpdate("finalHeading", value)} required max={100} /><ContentArea label="Body copy" value={copy.finalBody} onChange={(value) => trackedUpdate("finalBody", value)} required max={260} /></div>
        </FormSection>
      </div>
...
export function AboutContentEditor({ content }: { content: AboutPageContent }) {
  const [copy, setCopy] = useState({ opening: editorialLinesToText(content.opening.heading), prefix: content.opening.experiencePrefix, experienceStatement: `${content.opening.experienceValue} ${content.opening.experienceLabel}`, lead: content.distinction.lead, distinction: content.distinction.body, relationship: content.approach.items[0]?.description ?? "", ambition: content.ambition.body, leadership: content.people.entries[0]?.description ?? "", experience: content.people.entries[1]?.description ?? "", cta: content.ambition.cta.label });
  const update = (key: keyof typeof copy, value: string) => setCopy((current) => ({ ...current, [key]: value }));
  const document = (): AboutPageContent => {
    const experienceParts = copy.experienceStatement.trim().split(/\s+/);
    return { ...content,
      opening: { ...content.opening, heading: textToEditorialLines(copy.opening), experiencePrefix: copy.prefix, experienceValue: experienceParts.shift() || content.opening.experienceValue, experienceLabel: experienceParts.join(" ") || content.opening.experienceLabel },
  ...
  return <>
    ...
    <div className="admin-editor-layout"><div>
      <FormSection eyebrow="01 / OPENING" title="Company positioning"><EditorialContentArea label="Editorial heading" value={copy.opening} onChange={(value) => trackedUpdate("opening", value)} required max={100} /><div className="admin-fields admin-fields--two"><Field label="Experience prefix"><TextInput value={copy.prefix} onChange={(event) => trackedUpdate("prefix", event.target.value)} /></Field><Field label="Experience statement"><TextInput value={copy.experienceStatement} onChange={(event) => trackedUpdate("experienceStatement", event.target.value)} /></Field></div></FormSection>
...
export function ContactContentEditor({ content }: { content: ContactPageContent }) {
  const [copy, setCopy] = useState({ opening: editorialLinesToText(content.opening.heading), openingBody: content.opening.body, formHeading: editorialLinesToText(content.formIntro.heading), helper: content.formIntro.helper, submitLabel: content.formIntro.quoteForm.submitLabel, messageLabel: content.formIntro.quoteForm.messageLabel, reassurance: content.reassurance.body });
  const update = (key: keyof typeof copy, value: string) => setCopy((current) => ({ ...current, [key]: value }));
  const document = (): ContactPageContent => ({ ...content,
    opening: { ...content.opening, heading: textToEditorialLines(copy.opening), body: copy.openingBody },
    formIntro: { ...content.formIntro, heading: textToEditorialLines(copy.formHeading), helper: copy.helper, quoteForm: { ...content.formIntro.quoteForm, submitLabel: copy.submitLabel, messageLabel: copy.messageLabel } },
  ...
  return <>
    ...
    <div className="admin-editor-layout"><div>
      <FormSection eyebrow="01 / OPENING" title="Direct conversation"><EditorialContentArea label="Heading" value={copy.opening} onChange={(value) => trackedUpdate("opening", value)} required max={100} /><ContentArea label="Supporting copy" value={copy.openingBody} onChange={(value) => trackedUpdate("openingBody", value)} required max={280} /></FormSection>
      <FormSection eyebrow="02 / QUOTE FORM" title="Requirement helper" description="The form fields themselves remain structured and design-controlled for later enquiry handling."><EditorialContentArea label="Form heading" value={copy.formHeading} onChange={(value) => trackedUpdate("formHeading", value)} required max={100} /><ContentArea label="Helper text" value={copy.helper} onChange={(value) => trackedUpdate("helper", value)} required max={250} /><div className="admin-fields admin-fields--two"><Field label="Form submit label"><TextInput value={copy.submitLabel} onChange={(event) => trackedUpdate("submitLabel", event.target.value)} /></Field><Field label="Message label"><TextInput value={copy.messageLabel} onChange={(event) => trackedUpdate("messageLabel", event.target.value)} /></Field></div></FormSection>
...
export function OperationsContentEditor({ content, media }: { content: OperationsPageContent; media: MediaAsset[] }) {
  const [copy, setCopy] = useState({ opening: editorialLinesToText(content.opening.heading), openingBody: content.opening.body, sequence: content.sequence.heading, coverage: editorialLinesToText(content.coverage.heading), coverageBody: content.coverage.body, transportLead: content.transport.lead, transportBody: content.transport.body, finalHeading: editorialLinesToText(content.finalCTA.heading), finalCta: content.finalCTA.cta.label });
  const [imageId, setImageId] = useState(content.opening.media.id);
  const update = (key: keyof typeof copy, value: string) => setCopy((current) => ({ ...current, [key]: value }));
  const document = (): OperationsPageContent => ({ ...content,
    opening: { ...content.opening, heading: textToEditorialLines(copy.opening), body: copy.openingBody, media: media.find((asset) => asset.id === imageId) ?? content.opening.media },
    sequence: { ...content.sequence, heading: copy.sequence },
    coverage: { ...content.coverage, heading: textToEditorialLines(copy.coverage), body: copy.coverageBody },
    transport: { ...content.transport, lead: copy.transportLead, body: copy.transportBody },
    finalCTA: { ...content.finalCTA, heading: textToEditorialLines(copy.finalHeading), cta: { ...content.finalCTA.cta, label: copy.finalCta } },
  });
  ...
  return <>
    ...
    <div className="admin-editor-layout"><div>
      <FormSection eyebrow="01 / WANI STOCKING FACILITY" title="Physical presence" description="Use this section for the Wani facility, without adding unknown capacity or equipment details."><EditorialContentArea label="Heading" value={copy.opening} onChange={(value) => trackedUpdate("opening", value)} required max={110} /><ContentArea label="Body copy" value={copy.openingBody} onChange={(value) => trackedUpdate("openingBody", value)} required max={320} /><MediaPicker media={media} selectedId={imageId} onSelect={(asset) => { setImageId(asset.id); editor.markDirty(); }} /></FormSection>
      ...
      <FormSection eyebrow="03 / GEOGRAPHIC EXPERIENCE" title="Coverage framing"><div className="admin-fields admin-fields--two"><EditorialContentArea label="Heading" value={copy.coverage} onChange={(value) => trackedUpdate("coverage", value)} required max={110} /><ContentArea label="Body copy" value={copy.coverageBody} onChange={(value) => trackedUpdate("coverageBody", value)} required max={300} /></div></FormSection>
      ...
      <FormSection eyebrow="05 / FINAL CALL TO ACTION" title="Next conversation"><div className="admin-fields admin-fields--two"><EditorialContentArea label="Heading" value={copy.finalHeading} onChange={(value) => trackedUpdate("finalHeading", value)} required max={100} /><Field label="Action label"><TextInput value={copy.finalCta} onChange={(event) => trackedUpdate("finalCta", event.target.value)} /></Field></div></FormSection>
