...
export function HomeContentEditor({ content, media }: { content: HomePageContent; media: MediaAsset[] }) {
  ...
  const toLines = (value: string, original: readonly { text: string }[]) => value === lineText(original) ? original : [{ text: value }];
  const update = (key: keyof typeof copy, value: string) => setCopy((current) => ({ ...current, [key]: value }));
  const document = (): HomePageContent => ({
    ...content,
    hero: { ...content.hero, meta: (() => { const [first, second] = copy.heroMeta.split("·").map((item) => item.trim()); return [first || content.hero.meta[0], second || content.hero.meta[1]] as [string, string]; })(), kicker: copy.heroKicker, heading: toLines(copy.heroHeading, content.hero.heading), primaryCTA: { ...content.hero.primaryCTA, label: copy.primaryLabel }, secondaryCTA: { ...content.hero.secondaryCTA, label: copy.secondaryLabel }, media: media.find((asset) => asset.id === heroMediaId) ?? content.hero.media },
    capabilityIntro: { ...content.capabilityIntro, heading: toLines(copy.capabilityHeading, content.capabilityIntro.heading), body: copy.capabilityBody },
    ...
    experience: { ...content.experience, heading: toLines(copy.experienceHeading, content.experience.heading), body: copy.experienceBody },
    finalCTA: { ...content.finalCTA, heading: toLines(copy.finalHeading, content.finalCTA.heading), body: copy.finalBody },
  });
    ...
    <div className="admin-editor-layout">
      <div>
        <FormSection eyebrow="01 / HERO" title="The first impression" description="The headline is intentionally short so it retains its editorial scale on every screen.">
          ...
          <ContentArea label="Headline" value={copy.heroHeading} onChange={(value) => trackedUpdate("heroHeading", value)} required max={80} />
        ...
        <FormSection eyebrow="02 / CAPABILITY INTRODUCTION" title="Supply context" description="Introduces the scale and requirement-led nature of the business without presenting a fixed catalogue.">
          <ContentArea label="Heading" value={copy.capabilityHeading} onChange={(value) => trackedUpdate("capabilityHeading", value)} required max={120} /><ContentArea label="Body copy" value={copy.capabilityBody} onChange={(value) => trackedUpdate("capabilityBody", value)} required max={300} />
        </FormSection>
        ...
        <FormSection eyebrow="08 / EXPERIENCE" title="New venture, established experience" description="This distinction must remain factual: Star Energies is a new business backed by industry experience.">
          <div className="admin-fields admin-fields--two"><ContentArea label="Heading" value={copy.experienceHeading} onChange={(value) => trackedUpdate("experienceHeading", value)} required max={120} /><ContentArea label="Body copy" value={copy.experienceBody} onChange={(value) => trackedUpdate("experienceBody", value)} required max={300} /></div>
        </FormSection>
        ...
        <FormSection eyebrow="09 / FINAL CALL TO ACTION" title="Enquiry moment" description="Keeps direct contact available as an alternative to the quote form.">
          <div className="admin-fields admin-fields--two"><ContentArea label="Heading" value={copy.finalHeading} onChange={(value) => trackedUpdate("finalHeading", value)} required max={100} /><ContentArea label="Body copy" value={copy.finalBody} onChange={(value) => trackedUpdate("finalBody", value)} required max={260} /></div>
        </FormSection>
      </div>
...
export function AboutContentEditor({ content }: { content: AboutPageContent }) {
  const [copy, setCopy] = useState({ opening: lineText(content.opening.heading), prefix: content.opening.experiencePrefix, experienceStatement: `${content.opening.experienceValue} ${content.opening.experienceLabel}`, lead: content.distinction.lead, distinction: content.distinction.body, relationship: content.approach.items[0]?.description ?? "", ambition: content.ambition.body, leadership: content.people.entries[0]?.description ?? "", experience: content.people.entries[1]?.description ?? "", cta: content.ambition.cta.label });
  const update = (key: keyof typeof copy, value: string) => setCopy((current) => ({ ...current, [key]: value }));
  const document = (): AboutPageContent => {
    const experienceParts = copy.experienceStatement.trim().split(/\s+/);
    return { ...content,
      opening: { ...content.opening, heading: copy.opening === lineText(content.opening.heading) ? content.opening.heading : [{ text: copy.opening }], experiencePrefix: copy.prefix, experienceValue: experienceParts.shift() || content.opening.experienceValue, experienceLabel: experienceParts.join(" ") || content.opening.experienceLabel },
...
export function ContactContentEditor({ content }: { content: ContactPageContent }) {
  ...
  const document = (): ContactPageContent => ({ ...content,
    opening: { ...content.opening, heading: copy.opening === lineText(content.opening.heading) ? content.opening.heading : [{ text: copy.opening }], body: copy.openingBody },
    formIntro: { ...content.formIntro, heading: copy.formHeading === lineText(content.formIntro.heading) ? content.formIntro.heading : [{ text: copy.formHeading }], helper: copy.helper, quoteForm: { ...content.formIntro.quoteForm, submitLabel: copy.submitLabel, messageLabel: copy.messageLabel } },
  ...
  return <>
    ...
    <div className="admin-editor-layout"><div>
      <FormSection eyebrow="01 / OPENING" title="Direct conversation"><ContentArea label="Heading" value={copy.opening} onChange={(value) => trackedUpdate("opening", value)} required max={100} /><ContentArea label="Supporting copy" value={copy.openingBody} onChange={(value) => trackedUpdate("openingBody", value)} required max={280} /></FormSection>
      <FormSection eyebrow="02 / QUOTE FORM" title="Requirement helper" description="The form fields themselves remain structured and design-controlled for later enquiry handling."><ContentArea label="Form heading" value={copy.formHeading} onChange={(value) => trackedUpdate("formHeading", value)} required max={100} /><ContentArea label="Helper text" value={copy.helper} onChange={(value) => trackedUpdate("helper", value)} required max={250} /><div className="admin-fields admin-fields--two"><Field label="Form submit label"><TextInput value={copy.submitLabel} onChange={(event) => trackedUpdate("submitLabel", event.target.value)} /></Field><Field label="Message label"><TextInput value={copy.messageLabel} onChange={(event) => trackedUpdate("messageLabel", event.target.value)} /></Field></div></FormSection>
...
export function OperationsContentEditor({ content, media }: { content: OperationsPageContent; media: MediaAsset[] }) {
  ...
  const document = (): OperationsPageContent => ({ ...content,
    opening: { ...content.opening, heading: copy.opening === lineText(content.opening.heading) ? content.opening.heading : [{ text: copy.opening }], body: copy.openingBody, media: media.find((asset) => asset.id === imageId) ?? content.opening.media },
    sequence: { ...content.sequence, heading: copy.sequence },
    coverage: { ...content.coverage, heading: copy.coverage === lineText(content.coverage.heading) ? content.coverage.heading : [{ text: copy.coverage }], body: copy.coverageBody },
    transport: { ...content.transport, lead: copy.transportLead, body: copy.transportBody },
    finalCTA: { ...content.finalCTA, heading: copy.finalHeading === lineText(content.finalCTA.heading) ? content.finalCTA.heading : [{ text: copy.finalHeading }], cta: { ...content.finalCTA.cta, label: copy.finalCta } },
  });
  ...
  return <>
    ...
    <div className="admin-editor-layout"><div>
      ...
      <FormSection eyebrow="03 / GEOGRAPHIC EXPERIENCE" title="Coverage framing"><div className="admin-fields admin-fields--two"><ContentArea label="Heading" value={copy.coverage} onChange={(value) => trackedUpdate("coverage", value)} required max={110} /><ContentArea label="Body copy" value={copy.coverageBody} onChange={(value) => trackedUpdate("coverageBody", value)} required max={300} /></div></FormSection>
      ...
      <FormSection eyebrow="05 / FINAL CALL TO ACTION" title="Next conversation"><div className="admin-fields admin-fields--two"><ContentArea label="Heading" value={copy.finalHeading} onChange={(value) => trackedUpdate("finalHeading", value)} required max={100} /><Field label="Action label"><TextInput value={copy.finalCta} onChange={(event) => trackedUpdate("finalCta", event.target.value)} /></Field></div></FormSection>
